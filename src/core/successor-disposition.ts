...
export type SuccessorDispositionV1 =
  | {
      schema_version: 1;
      ...
      next_task: {
        ...
        source_artifact_identity: `sha256:${string}`;
      };
      no_successor: null;
    }
  | {
...
export function buildSuccessorDisposition(
  input: Omit<SuccessorDispositionV1, "schema_version" | "record_kind" | "record_id">
): SuccessorDispositionV1 {
  if (!input.source_run_instance_id.trim()) throw new Error("successor_disposition_missing_run_instance");
  const identity = { schema_version: 1, record_kind: "successor_disposition", ...input };
  return {
    ...identity,
    record_id: `sha256:${sha256Hex(canonicalJson(identity))}`
  } as SuccessorDispositionV1;
}

export function assertCompatibleSuccessorDisposition(
  existing: SuccessorDispositionV1 | undefined,
  proposed: SuccessorDispositionV1
): void {
